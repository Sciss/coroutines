...
private[coroutines] class ByTreeTyper[C <: whitebox.Context](val c: C)(val treeValue: Any) {
  import c.universe._
  ...
  val untypedTree: c.Tree = c.untypecheck(tree)
  ...
  object typeOf {
    ...
    def update(t: Tree, tpe: Type): Unit = augmentedTypes(t) = tpe
  }
}
